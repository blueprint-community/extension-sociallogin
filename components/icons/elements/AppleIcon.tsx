import React from 'react';
import IconProps from '../IconProps'

const Icon: React.FC<IconProps> = ({ className, alt, color }) => (
    <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        className={className+' SocialLogin:Glyph'}
        aria-labelledby={alt}
        color={color || "currentColor"}
        fill={"none"}
    >
        <path d="M12 6.25C12 4.25 13.5 2.25 15.5 2.25C15.5 4.25 14 6.25 12 6.25Z" fill="currentColor" />
        <path d="M12.5 7.59001C11.9851 7.59001 11.5867 7.42646 11.1414 7.24368C10.5776 7.01225 9.93875 6.75 8.89334 6.75C7.02235 6.75 4 8.24945 4 12.2495C4 16.9016 7.10471 21.75 9.10471 21.75C9.77426 21.75 10.3775 21.4871 10.954 21.2359C11.4815 21.0059 11.9868 20.7857 12.5 20.7857C13.0132 20.7857 13.5185 21.0059 14.046 21.2359C14.6225 21.4871 15.2257 21.75 15.8953 21.75C17.2879 21.75 18.9573 19.3992 20 16.4008C18.3793 15.7202 17.338 14.118 17.338 12.25C17.338 10.621 18.2036 9.53982 19.5 8.75C18.5 7.25 17.0134 6.75 15.9447 6.75C14.8993 6.75 14.2604 7.01225 13.6966 7.24368C13.2514 7.42646 13.0149 7.59001 12.5 7.59001Z" fill="currentColor" />
    </svg>
);

export default Icon;